  ...
  return (nodesAccessor, cursorOfInitialNode, {
    ...
    const executeFilterQuery = query => operateOverScalarOrArray(query, orderColumn, (orderBy, index, prev) => {
      ...
      if (index > 0) {
        ...
        const nested = prev[operation](`(?? = ? and ?? ${comparator} ?)`, [formatColumnIfAvailable(orderColumn[index - 1], formatColumnFn), values[index - 1], formatColumnIfAvailable(orderBy, formatColumnFn), values[index]]);
        return nested;
      }
      ...
      const lastValue = columnValue.pop(); // If value is null, we are forced to filter by id instead

      const operation = isAggregateFn && isAggregateFn(lastOrderColumn) ? 'orHavingRaw' : 'orWhereRaw';

      if (lastValue === null || lastValue === undefined) {
        return prev[operation](`(?? ${comparator} ?) or (?? IS NOT NULL)`, [formatColumnIfAvailable('id', formatColumnFn), id, formatColumnIfAvailable(lastOrderColumn, formatColumnFn)]);
      }

      return prev[operation](`(?? = ? and ?? ${comparator} ?)`, [formatColumnIfAvailable(lastOrderColumn, formatColumnFn), lastValue, formatColumnIfAvailable('id', formatColumnFn), id]);
    });

    let result;

    if (isAggregateFn && Array.isArray(orderColumn) && isAggregateFn(orderColumn[0]) || isAggregateFn && !Array.isArray(orderColumn) && isAggregateFn(orderColumn)) {
...
const orderNodesBy = (nodesAccessor, {
  ...
  }, (prev, isArray) => isArray ? prev.orderBy(formatColumnIfAvailable('id', formatColumnFn, false), ascOrDesc[0]) : prev.orderBy(formatColumnIfAvailable('id', formatColumnFn, false), ascOrDesc));
  return result;
...
const removeNodesBeforeAndIncluding = buildRemoveNodesFromBeforeOrAfter('before'); // Used when `first` is included in the query
...
const removeNodesFromEnd = (nodesAccessor, first) => nodesAccessor.clone().limit(first); // Used when `before` is included in the query
...
const removeNodesAfterAndIncluding = buildRemoveNodesFromBeforeOrAfter('after'); // Used when `last` is included in the query
...
const removeNodesFromBeginning = (nodesAccessor, last, {
  orderColumn,
  ascOrDesc
}) => {
  const invertedOrderArray = operateOverScalarOrArray([], ascOrDesc, (orderDirection, index, prev) => prev.concat(orderDirection === 'asc' ? 'desc' : 'asc'));
  ...
  const result = nodesAccessor.clone().from(subquery.as('last_subquery')).clear('select').clear('where').clear('join').clear('with');
  return result;
};

const getNodesLength = async nodesAccessor => {
  const counts = await nodesAccessor.clone().clear('select').count('*');
...
const convertNodesToEdges = (nodes, _, {
  orderColumn
}) => {console.debug(nodes);return nodes.map(node => {
